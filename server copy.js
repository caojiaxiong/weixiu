...
app.get('/charts', async (req, res) => {
    try {
        ...
        const pageSize = parseInt(req.query.pageSize) || 20; // 默认每页20条
        const lastMessageId = req.query.lastMessageId; // 上次获取的最后一条消息ID（用于向上滑动加载更多）
        ...
        let paginatedData = [];
        let totalCount = sortedCharts.length;
        let hasMore = false;
        
        if (lastMessageId) {
            // 向上滑动加载更多（获取更早的消息）
            const lastIndex = sortedCharts.findIndex(msg => msg.id === lastMessageId);
            if (lastIndex !== -1) {
                const startIndex = lastIndex + 1;
                const endIndex = Math.min(startIndex + pageSize, sortedCharts.length);
                paginatedData = sortedCharts.slice(startIndex, endIndex);
                hasMore = endIndex < sortedCharts.length;
            }
        } else {
            // 正常分页
